...
void CFhelp::Help()
{
	if (updHelp > 0 )
	{
		...
		Fclr = CLR( 1, 0.8f, 0.9f,  1 );
			scpy(s, "Page:" );		Write( xf2/2 - 40, y );
		
		Fclr = CLR( 1.0f, 0.7f, 0.95f,  1 );
			format(s,S, "%c %d %c",
				(HelpPage > 0) ? '<' : ' ',
				 HelpPage + 1,
				(HelpPage < HelpPages) ? '>' : ' ' );
		...
		/*  Main  */
			...
			/**/y = yy;  Fclr = CLR( 0.6f, 1, 1,  1 );	
				...
				scpy(s, "view char / maximize display" );	Write( x0, y );  y += Ly;
		...
		/*  Table  */
			case 1:
				...
				scpy(s, "F1..F6" );			WriteL(x, y); y += Ly*3/2;
			...
			/**/y = yy + 2*Ly;
				...
				scpy(s, "Color Font Glow Alpha save edit" );	Write( x1+120, y );	y += Ly*3/2;	Fclr = CLR( 0.6f, 1, 1,  1 );	
